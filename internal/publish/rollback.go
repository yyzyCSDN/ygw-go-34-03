...
func (p *Publisher) Rollback(ctx context.Context, app model.AppID, group model.GroupID, to int64) model.Result {
	...
	rev := p.st.Revision()
	restored := RestoreBatch(rec.BatchID, app, group, rec.Entries)
	txn := store.NewTxn(restored.ID, ns, restored.Entries, rev+1)
	_ = p.st.Commit(txn)
	snap := p.st.Snapshot(app, group)
	p.versions.Record(version.RecordFromSnapshot(snap.Revision, ns, snap))

	p.journal.Append(audit.NewRecord(model.EventRollback, snap.Revision, rec.BatchID, app, group, snap.Checksum))
